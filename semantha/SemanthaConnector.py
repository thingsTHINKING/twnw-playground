...
class SemanthaConnector:
    ...
    def __init__(self, server_base_url="http://localhost/tt-platform-server", streamlit_domains_prefix="",
                 api_key=None):
    ...
    def query_library(self, text: str, domain: str, threshold=0.75, max_references=10, tags=None):
    ...
    def do_semantic_string_compare(self, input_0: str, input_1: str, domain: str) -> float:
    ...
    def get_opposite_meaning(self, input_0: str, input_1: str, domain: str) -> bool:
        doc = self.__get_references(input_0, input_1, domain, "23d06b42-4a32-4531-b00e-640f538e2aee")
        return doc.pages[0].contents[0].paragraphs[0].references \
            and doc.pages[0].contents[0].paragraphs[0].references[0].has_opposite_meaning

    def change_model(self, domain: str, model_id: int) -> int:
        return int(self.__sdk.domains(domain).settings
                   .patch(PatchDomainSettings(similarity_model_id=str(model_id))).similarity_model_id)

    def __get_references(self, input_0, input_1, domain, doc_type=None):
        return self.__sdk.domains(domain).references.post(file=_to_text_file(input_0),
