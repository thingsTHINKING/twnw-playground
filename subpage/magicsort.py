...
TOPICS_OVER_TIME = ['startups', 'newsticker']
USE_CASES = {'Startups': 'startups', 'M&A Newsticker': 'newsticker', 'UK Legislations': 'uk_legislations', 'GRI vs. ESRS': 'gri_vs_esrs',  'UK Manifestos': 'uk_manifestos', 'US Investment Bill': 'bill'}
...
class MagicSort(AbstractPage):
    ...
    def build(self):
        ...
        "Which use case would you like to see?",
        (USE_CASES.keys()))
        self._use_case = USE_CASES[use_case]
    ...
    def _visualize_clustering(self, granularity):
        with st.expander('Visualization 📚'):
            st.write('''
                    ...
                    ''')
            if self._use_case in TOPICS_OVER_TIME:
    ...
    def _show_sorted_documents(self, data, granularity):
        ...
        topics = pd.read_excel(f'data/magicsort/{self._use_case}/{granularity}/{granularity}_excel.xlsx')
    ...
    def _determine_granularity(self):
        st.write('A broad overview of the documents can be achieved by using a high granularity. A high granularity means that the documents are clustered into a few topics. A low granularity means that the documents are clustered into many topics.')
        option = st.selectbox(
    ...
    def _load_data(self):
        st.markdown('### Library')
        temp_dict = dict((v,k) for k,v in USE_CASES.items())
        st.write(f'This is your library of documents, in this instance, the documents are descriptions of {temp_dict[self._use_case]}. You can use MagicSort to get an overview over the documents and to find trends.')
    ...
    def _load_figure(self, type, granularity):
        with open(f'data/magicsort/{self._use_case}/{granularity}/{granularity}_{type}.json', 'r') as f:
            data = json.loads(f.read())
            return pio.from_json(data)

    def _topic_over_time_visualization(self, granularity):
        tab1, tab2, tab3 = st.tabs(['All Documents', 'Cluster', 'Topics over Time'])
